...
@lru_cache(maxsize=1)
def get_kalshi_instrument_provider(client: KalshiHttpClient, config: KalshiInstrumentProviderConfig | None=None) -> KalshiInstrumentProvider:
    return KalshiInstrumentProvider(http_client=client, config=config or KalshiInstrumentProviderConfig())
...
class KalshiLiveDataClientFactory(LiveDataClientFactory):
    ...
    def create(loop: asyncio.AbstractEventLoop, name: str, config: KalshiDataClientConfig, msgbus: MessageBus, cache: Cache, clock: LiveClock) -> KalshiDataClient:
        ...
        provider = get_kalshi_instrument_provider(client=http_client, config=config.instrument_provider)
